...
class EmergencyStopCoordinator(DataUpdateCoordinator):
    ...
    async def _async_update_data(self):
        ...
        so entities can quickly look up their data.
        """
        _LOGGER.debug("EmergencyStopCoordinator _async_update_data")
        ...
        async with aiohttp.ClientSession() as session:
            try:
                ...
                _LOGGER.warn(f"Failed to update emergency stop info: {e}")
                #raise UpdateFailed(f"Error communicating with socket: {e}")